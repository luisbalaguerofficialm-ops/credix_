...
export default function ResetTransactionPin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axiosClient.put("/api/users/transaction-pin/reset", {
        newPin: pin.newPin,
      ...
      toast.error(
        error.response?.data?.message || "Unable to update transaction PIN.",
      );
    } finally {
      setLoading(false);
    }
          ...
          <div className="w-full max-w-lg bg-[#FFFFFF] rounded-2xl shadow-[0px_10px_30px_rgba(0,0,0,0.04)] border border-[#e2e8f0] p-10 md:p-12 relative overflow-hidden">
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              ...
              <div className="flex flex-col gap-3 pt-2">
                <button
                  className="w-full h-13 bg-[#00516f] text-white font-semibold text-[16px] rounded-xl shadow-sm hover:bg-[#003f57] transition-all flex items-center justify-center disabled:opacity-70 disabled:cursor-not-allowed"
                  type="submit"
                  disabled={loading}
                >
                  {loading ? (
